...
export const createSite = async (c: Context<{ Bindings: Env }>) => {
  try {
    ...
    const userClaimEmail =
      jwtPayload?.claims?.email || jwtPayload?.claims?.primary_email_address || null;
    const firstNotificationEmail = body.notification_emails
      ? body.notification_emails.split(',')[0].trim()
      : null;
    const adminEmail =
      body.admin_email?.trim() ||
      firstNotificationEmail ||
      userClaimEmail ||
      `admin@${cleanDomain}`;
      ...
      if (userId) {
        ...
        if (userCompany?.id) {
          ...
          await c.env.DB.prepare(`
                    ...
                    `)
            .bind(companyId, 'My Workspace', userId)
            .run();
        }
      } else {
        const defaultComp = await c.env.DB.prepare(
          "SELECT id FROM companies WHERE id = 'comp_default'"
        ).first<{ id: string }>();
    ...
    const notifyOnSubmission =
      body.notify_on_submission === undefined ? 1 : body.notify_on_submission ? 1 : 0;
