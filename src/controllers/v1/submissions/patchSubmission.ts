...
import { PatchSubmissionBody } from '../../../schemas/submission.schema';
...
export const patchSubmission = async (c: Context<{ Bindings: Env }>) => {
    try {
        ...
        if (!body.status || !validStatuses.includes(body.status)) {
            return sendProblemDetails(c, 422, `Invalid status. Must be one of: ${validStatuses.join(', ')}`, {
                invalidParams: [{ name: 'status', reason: `Must be one of: ${validStatuses.join(', ')}` }],
            });
        }
