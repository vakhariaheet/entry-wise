...
export async function processSubmissionDelivery(
    ...
    const errors: string[] = [];
    const { submissionId, siteId, companyId, fields, savedAttachmentMeta, submitterEmail, submitterName } = message;

    try {
        // 1. Fetch Site Configuration
        const site = await env.DB.prepare('SELECT * FROM sites WHERE id = ?').bind(siteId).first<any>();
        if (!site) {
            console.error(`[Queue Processor] Site '${siteId}' not found for submission '${submissionId}'`);
        ...
        console.error(`[Queue Processor] Unexpected failure processing submission '${submissionId}':`, err);
