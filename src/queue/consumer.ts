...
export async function queueConsumer(
  ...
  console.log(
    `[Queue Consumer] Processing batch of ${batch.messages.length} messages from queue: ${batch.queue}`
  );
  ...
  for (const message of batch.messages) {
    try {
      ...
      console.log(
        `[Queue Consumer] Processing submission ID '${payload.submissionId}' (attempt: ${message.attempts})`
      );
      ...
      if (result.success) {
        console.log(
          `[Queue Consumer] Successfully delivered submission ID '${payload.submissionId}'`
        );
        message.ack();
      } else {
        console.warn(
        ...
        if (message.attempts < 3) {
          const delaySeconds = Math.min(300, 2 ** message.attempts * 10);
          console.log(
            `[Queue Consumer] Retrying submission ID '${payload.submissionId}' in ${delaySeconds}s (attempt ${message.attempts})`
          );
