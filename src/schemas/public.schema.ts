...
import { MAX_FILE_SIZE, VALID_FILE_TYPES } from '../types/submission';
...
const validateFieldValue = (type: string) => {
  switch (type) {
    ...
    case 'file':
      return z
        .instanceof(File)
        .refine((file) => VALID_FILE_TYPES.includes(file.type), 'Invalid file type')
        .refine((file) => file.size <= MAX_FILE_SIZE, 'File size exceeds limit')
        .openapi({
