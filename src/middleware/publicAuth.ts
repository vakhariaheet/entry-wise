...
export const verifyDomain = async (c: Context<{ Bindings: Env }>, next: Next) => {
  try {
    ...
    const { results } = await c.env.DB.prepare(`
        ...
        `)
      .bind(apiKey)
      .all<{ id: string; company_id: string; domain: string; allowed_origins?: string }>();
    ...
    if (originHeader) {
      try {
        ...
        let isAllowed =
          cleanRequestHost === siteHostname ||
          cleanRequestHost.endsWith(`.${siteHostname}`) ||
          isLocalhost;
          ...
          if (allowedList.includes('*')) {
            ...
            for (const item of allowedList) {
              try {
                ...
                if (
                  cleanRequestHost === allowedHost ||
                  cleanRequestHost.endsWith(`.${allowedHost}`)
                ) {
                  isAllowed = true;
                  break;
                }
...
export const corsMiddleware = async (c: Context<{ Bindings: Env }>, next: Next) => {
  ...
  if (origin) {
    ...
    c.res.headers.set(
      ...
      'Content-Type, X-API-Key, cf-turnstile-response, Authorization'
    );
