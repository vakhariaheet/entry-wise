...
class ApiService {
  ...
  async listCompanies(): Promise<Company[]> {
    ...
    if (!res.ok) {
      ...
      throw new Error(err.detail || err.message || `Failed to fetch workspaces (HTTP ${res.status})`);
    }
    const json = await res.json();
    return json.items || json.data || [];
  }

  async createCompany(data: { name: string; email_provider?: string; from_email?: string; from_name?: string }): Promise<Company> {
    const res = await fetch(`${API_BASE}/companies`, {
      method: 'POST',
      headers: this.getAuthHeaders(),
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || err.message || `Failed to create workspace (HTTP ${res.status})`);
    }
    const json = await res.json();
  ...
  async updateCompany(id: string, data: Partial<Company>): Promise<Company> {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || err.message || `Failed to update workspace (HTTP ${res.status})`);
    }
    const json = await res.json();
    return json.data || json;
  }
  ...
  async deleteCompany(id: string): Promise<void> {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || err.message || `Failed to delete workspace (HTTP ${res.status})`);
    }
  }

  // Sites (Forms)
  async listSites(companyId?: string): Promise<Site[]> {
    const url = companyId ? `${API_BASE}/sites?company_id=${encodeURIComponent(companyId)}` : `${API_BASE}/sites`;
  ...
  async listSubmissions(
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || err.message || `Failed to fetch submissions (HTTP ${res.status})`);
    }
    const json = await res.json();
    return {
  ...
  async patchSubmissionStatus(
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || err.message || `Failed to update submission status (HTTP ${res.status})`);
    }
  }
  ...
  async deleteSubmission(siteId: string, submissionId: string): Promise<void> {
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
      throw new Error(err.detail || err.message || `Failed to delete submission (HTTP ${res.status})`);
    }
  }
